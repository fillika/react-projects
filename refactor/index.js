...
const load_images = function (slide_id) {
  ...
  for (j = 0; j < $('#slide_' + slide_id + ' .img').length; j++) {
    $($('#slide_' + slide_id + ' .img')[j])
      .children('img')
  ...
  $(slide).on('load', function () {
    ...
    if (loaded_slides === $('.slide_block').length) {
      ...
      $('#navigation_slide .text_nav a, .block_nav nav a.slide_0')
        .removeClass('unready')
        .on('click', function () {
...
function sl_btn_click(elem) {
  ...
  function slide_up_down(elem, properties) {
    ...
    effects[id].func = function () {
      ...
      $($('#' + effects[id].slide_number + ' .' + effects[id].elem)[0]).animate(
  ...
  if ($('#' + slide_number).hasClass('second')) {
